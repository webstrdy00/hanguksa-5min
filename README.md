...
- SDK 식별키 획득과 각 HTTP 요청에는 각각 30초 제한시간을 둡니다(응답 본문 수신 포함).
  시간 초과 시 진입 화면에 안내와 재시도 버튼을 표시하고, 늦게 도착한 이전 인증 결과는 적용하지 않습니다.
  HTTP 요청은 중단하지만 이미 서버에서 처리한 쓰기를 되돌리는 것은 아닙니다.
  이 제한은 무한 로딩을 막을 뿐, 무료 서버의 콜드스타트를 없애지는 않습니다.

### 운영 환경 전환 체크리스트

서버는 기동 직후 삭제 큐를 처리하고, 각 배치 종료 30초 후 최대 50건을 다시 처리합니다.
실패·중단 작업도 재시도하며, 트랜잭션과 행 잠금으로 중복 처리를 방지합니다.
종료 시 진행 중인 삭제를 기다린 뒤 DB 연결을 닫습니다.
`deletion_job_failed` / `deletion_worker_failed` 로그는 운영 알림 연결 대상입니다.
Render 무료 인스턴스가 절전 중이면 작업도 멈추며, 재기동 시 다시 처리합니다.
이는 24시간 상시 처리 보장이 아닙니다. 백업 소거는 별도 보관 정책에 따르며 여전히 확정이 필요합니다.
